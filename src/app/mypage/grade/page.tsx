...
/**
 * 회원등급 페이지 — 시안 37:12695 매칭
 *  - 좌측: MyPageSideNav
 *  - 우측 상단 좌측: 사용가능한 적립금 카드 (포인트 + 예상/소멸 예정)
 *  - 우측 상단 우측: 현재 등급 카드 (STANDARD 등 + 다음 등급까지 진행바)
 *  - 우측 하단: 회원등급별 혜택안내 5단계 비교 테이블 (브론즈/실버/골드/다이아/VIP)
 *
 * NOTE: 백엔드 등급/적립금 API 미구현 → 더미 값. /api/v1/me/points·grade 추가 시 교체.
 */
export default function MemberGradePage() {
    const { user, loading: authLoading } = useAuth();
    const router = useRouter();
    const [points] = useState({ available: 1000, expecting: 0, expiring30d: 0 });
    const [grade] = useState<{ name: GradeKey; spent: number; nextThreshold: number; }>({
        name: "STANDARD",
        spent: 60_000,
    ...
    useEffect(() => {
        if (!authLoading && !user) router.replace("/login?redirect=/mypage/grade");
    ...
    const progress = Math.min(100, (grade.spent / grade.nextThreshold) * 100);
            ...
            <section>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="rounded-[var(--radius-lg)] bg-[var(--color-bg-subtle)] p-5 md:p-6">
                        ...
                        <div className="relative h-2 rounded-full bg-[var(--color-bg-subtle)] overflow-hidden">
                            <div
                        ...
                        <div className="mt-1 flex justify-between text-[10px] text-[var(--color-fg-subtle)] tabular-nums">
                            <span>0원</span>
                            <span>{grade.nextThreshold.toLocaleString()}원</span>
                        </div>
                    </div>
                </div>
...
function GradeBadge({ name }: { name: GradeKey }) {
    return (
        <div className="w-12 h-12 rounded-full bg-[var(--color-bg-subtle)] flex items-center justify-center text-[var(--color-fg-muted)]" aria-label={name}>
